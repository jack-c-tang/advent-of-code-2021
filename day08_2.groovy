def inputs = ["gfeabcd adecfb gcedb cef efgdc decabg cbfg edfga fdegcb cf | fc faedg fdage fec",
...
def out = 0
inputs.each { input ->
    def samples = input.tokenize(' | ')[0..9]
    def outputs = input.tokenize(' | ')[10..13]
    def digits = [:]
    samples.each { sample ->
        def sl = sample.split('')
        if (sample.size() == 2) {
    ...
    outputs.eachWithIndex { output, index ->
        ...
        if (output.size() == 2) {
            ...
            if ((ol - digits[1]).size() == 5) {
                number = 6
            } else if ((ol - digits[4]).size() == 3) {
                number = 0
            } else {
                number = 9
            }
        } else if (output.size() == 5) {
            if ((ol - digits[1]).size() == 3) {
                number = 3
            } else if ((ol - digits[4]).size() == 3) {
                number = 2
            } else {
